...
export default class PhotoService extends BaseService<Photo> {
    ...
    async adaptToModel(
        data: any,
        options: Partial<PhotoAdapterOptions> = {}
    ): Promise<Photo> {
    ...
    public async getById(photoId: number, options: Partial<PhotoAdapterOptions>): Promise<Photo | null> {
        return super.getByIdFromTable("photo", photoId, options);
    }

    public async getByVehicleId(vehicleId: number, options: Partial<PhotoAdapterOptions>): Promise<Photo[]> {
        return super.getByFieldIdFromTable("photo", "vehicle_id", vehicleId, options);
    }

    public async getPhotoByBrandModelAndYearAndColor(brandModelId: number, manufactureYear: number, paintColor: string, options = { loadChildren: false }): Promise<Photo | null> {
        ...
        return super.getOneByFieldsValueFromTable("photo", queryFields, { loadChildren: false });
    }
}